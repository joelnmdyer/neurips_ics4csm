...
def produce_loss(omega, calib, states, aggs, ths, i_s, model, loss_fn):
    ...
    for j in range(states.shape[0]):
        ...
        i = i_s[j]
        x = aggs[j:j+1]
        y = simulate_and_intervene(model, macro_th, i.item(), x[0,0]/400, aggs.shape[1] - 1).unsqueeze(0)
...
def train_epi(omega, calib, abm_states, abm_agg_ts, abm_thi,
              ...
              max_epochs=10, max_epochs_no_improve=30, prop_val=0.2, full_node=False,
              notebook=True, model=None):
