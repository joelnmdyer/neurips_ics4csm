...
from neurips_ics4csm.models import bwsg_ode
try:
    from neurips_ics4csm.models import bwsg
...
def instantiate_model(t):
    ...
    model = bwsg_ode.BWSGRicker(t)
    return model


# Instantiates the RNN
def create_instantiate_bwsgrnn(net):
    def instantiate_bwsgrnn(t):
        model = bwsg_ode.BWSGRNN(t, net)
        return model
    return instantiate_bwsgrnn
...
def generate_networks(kind='lodernn', seed=0):
    ...
    if kind in ['lrnn']:
        ...
        mlp_net = MLP(input_dim=16,
                        output_dim=4, 
                        hidden_dims=[16, 16, 8], final_nonlinearity=torch.nn.Sigmoid()).double()
        # This is to map from ODE output to hidden state of RNN
        rnn_net = RNN(input_size=1, hidden_size=16,  final_ff=mlp_net, flavour='gru').double()
        ...
        # This maps from ABM parameters to parameters of ODE
        mlp = MLP(input_dim=3, output_dim=3, hidden_dims=[32, 32, 32], final_nonlinearity=torch.nn.Softplus()).double()
    elif kind in ['lodernn']:
        #raise NotImplementedError
        mlp_net = MLP(input_dim=16, output_dim=4, hidden_dims=[16], final_nonlinearity=torch.nn.Sigmoid()).double()
        rnn_net = RNN(input_size=4, hidden_size=16, final_ff=mlp_net, flavour='gru').double()
        # This maps from ABM parameters to parameters of ODE
        mlp = MLP(input_dim=3, output_dim=3, hidden_dims=[32, 16, 16], final_nonlinearity=torch.nn.Softplus()).double()   
    omega = Omega(mlp)
...
def create_instantiate_emission(N, kind='lodernn'):
    def instantiate_emission(e_pars):
        if kind in ['lrnn', 'lodernn', 'lode']:
            return torch.distributions.binomial.Binomial(N, e_pars)
        #elif kind in ['lode']:
        #    return torch.distributions.binomial.Binomial(N, e_pars)
        #elif kind in ['lodernn']:
        #    raise NotImplementedError
...
def build_surrogate_compute_metric(instantiate_emission, omega, params, model, y0, i, rnn_net, x, N, T):
    ...
    assert y_mac_stoch.shape[0] == 51, y_mac_stoch.shape[1] == 4
    #y_mac_stoch_mean = torch.stack([e_d.mean for e_d in e_dists])
    # Stoch MSE
    this_stoch_mse_loss = mse_loss(x, y_mac_stoch)
    # MSE
    ...
    ll = 0.
    for t in range(T+1):
        term = e_dists[t].log_prob((x[t]).int())
        ll += term
...
def collect_metrics(xs_test, this_test, instantiate_emission, obs_omega, int_omega, model, obs_rnn_net, int_rnn_net, N, model2=None):
    ...
    with torch.no_grad():
        ...
        for r in range(R):
            this_test_x, i0, (sheep, wolf, bear), i = xs_test[r], xs_test[r, 0], this_test[r, :3], this_test[r, -1].item()
            y0 = i0 / 400.
            params = torch.tensor([sheep, wolf, bear])
...
def run_spatial_intervention(params, i, T, L):

    # TODO: change kwarg N to L here
    # params = [sheep_reproduce, wolf_reproduce, bear_reproduce]
    model = bwsg.BearWolfSheepGrass(width=L, 
                                    height=L, 
                                    sheep_reproduce=params[..., 0], 
                                    wolf_reproduce=params[..., 1], 
                                    bear_reproduce=params[..., 2])
    x = torch.tensor([model.schedule.get_type_count(bwsg.Bear),
                      model.schedule.get_type_count(bwsg.Wolf),
                      model.schedule.get_type_count(bwsg.Sheep),
                      model.schedule.get_type_count(bwsg.GrassPatch)]).unsqueeze(0)
    initial_state = x.clone()
    for t in range(T):    
        add_bears = 0
        if (i != 0) and (t == 10 + i - 1):
            add_bears = 10
        x_grid = model.step(add_bears)
        x_new = torch.tensor([model.schedule.get_type_count(bwsg.Bear),
                              model.schedule.get_type_count(bwsg.Wolf),
                              model.schedule.get_type_count(bwsg.Sheep),
                              model.schedule.get_type_count(bwsg.GrassPatch, lambda x: x.fully_grown)])
...
def check_surrogate_cost(family, dirname, obs_or_int, seed=0):
    ...
    if not family in ['ode', 'lodernn']:
        ...
        model = torch.load(os.path.join(dirname, "best_{1}_model_{2}.pt".format(family, obs_or_int, seed)))
    y0 = torch.tensor([0., 50., 100., 400.])/400.

    params = torch.rand((R, 3))*0.1
    ...
    for r in range(R):
        _ = run_spatial_intervention(params[r], r % 5, T, L)
